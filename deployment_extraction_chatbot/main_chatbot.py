...
def format_params_message(params: Dict[str, Any]) -> str:
    lines = []
    for key in ("deposit_amount", "loan_amount", "deposit_duration",
...
# 2) Initialize your LLM chain once
extraction_chain = extract_chain()

# 3) FastAPI app + CORS (so any frontend can call it)
app = FastAPI()
app.add_middleware(
    CORSMiddleware,
...
async def chat(req: ChatRequest):
    ...
    try:
        ...
        return ChatResponse(
            extracted_parameters_value=fallback,
            generated_message=random_irrelevant()
        )

    # 4b) Decide which template to use
